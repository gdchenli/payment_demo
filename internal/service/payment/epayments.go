...
func (e *Epayments) Pay(order validate.Order) (form string, errCode int, err error) {
	...
	if order.UserAgentType == consts2.MobileUserAgentType {
...
func (e *Epayments) getPaymentChannels(methodCode string) (paymentChannels string) {
	if methodCode == consts2.WechatMethod {
		paymentChannels = payment.ChannelWechat
	} else if methodCode == consts2.AlipayMethod {
...
func (e *Epayments) Notify(query, methodCode string) (notifyRsp response.NotifyRsp, errCode int, err error) {
	...
	defer func() {
		...
		logrus.Infof("order id:%v,org:%v,method:%v,notify data:%+v",
			epaymentsNotifyRsp.OrderId, consts2.EpaymentsOrg, methodCode, epaymentsNotifyRsp.Rsp)
	}()
...
func (e *Epayments) Verify(query, methodCode string) (verifyRsp response.VerifyRsp, errCode int, err error) {
	...
	defer func() {
		...
		logrus.Infof("order id:%v,org:%v,method:%v,callback data:%+v",
			epaymentsCallbackRsp.OrderId, consts2.EpaymentsOrg, methodCode, epaymentsCallbackRsp.Rsp)
	}()
...
func (e *Epayments) SearchTrade(orderId, methodCode, currency string, totalFee float64) (searchtradeRsp response.SearchTradeRsp, errCode int, err error) {
	...
	defer func() {
		//记录日志
		logrus.Infof("order id:%v,org:%v,method:%v,callback data:%+v",
			epaymentsTradeRsp.OrderId, consts2.EpaymentsOrg, methodCode, epaymentsTradeRsp.Rsp)
	}()
